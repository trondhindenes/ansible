...
class Connection(ConnectionBase):
    ...
    def execute_cmd(self, cmd, env=None, nolog=False, no_log_cmdline=False):
        #TODO: Clean this up, I just ripped it from some other repo
        if no_log_cmdline is False:
            print("Executing {}".format(cmd))
        else:
            print("Executing command (nolog is True)")
        proc_call = subprocess.Popen(cmd, shell=True, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout_buffer, stderr_buffer = [], []

        return_code = proc_call.wait()
        with proc_call.stdout:
            for line in iter(proc_call.stdout.readline, b''):
                if nolog is False:
                    stdout_line = line.decode('utf-8').replace('\n', '')
                    stdout_buffer.append(stdout_line)

        if no_log_cmdline is False:
            print("{} exited with returncode {}".format(cmd, return_code))
        else:
            print("Command exited with returncode {}".format(return_code))


        if return_code != 0:
            print("Errors logged:")
            try:
                for line in iter(proc_call.stderr.readline, b''):
    ...
    def exec_command(self, cmd, in_data=None, sudoable=True):
        ...
        self._display.vvvvv('EXEC_COMMAND', host=self._winrm_host)
        import tempfile

        #Create a temp file to invoke pwsh locally (which again sets up the remoting session)
        ps_wrapper, ps_wrapper_filename = tempfile.mkstemp(suffix=".ps1")
        wrapper_str = r"""param($HostName, $UserName, $Password, $ScriptFile)
        $SecPass = $Password | ConvertTo-SecureString -AsplainText -Force
        $Cred = New-Object System.Management.Automation.PSCredential ($UserName, $SecPass)
        Invoke-Command -ComputerName $HostName -Credential $Cred -Authentication Basic -UseSSL -File $ScriptFile
        """
        os.write(ps_wrapper, wrapper_str)
        os.close(ps_wrapper)

        #Stream the value of "in_data" into a temp file we can send to the target node
        ps_target, ps_target_filename = tempfile.mkstemp(suffix=".ps1")
        os.write(ps_target, in_data)
        os.close(ps_wrapper)

        # self._display.vvvvv('PWSH CREATE SESSION', host=self._winrm_host)
        # create_session_resptuple = self.execute_cmd("pwsh -ExecutionPolicy Bypass -NoProfile -NoLogo -File {} -HostName {} -UserName {} -Password {}".format(ps_create_session_str_filename, self._winrm_host, self._winrm_user, self._winrm_pass))
        # self._display.vvvvv('PWSH CREATE SESSION END', host=self._winrm_host)
        self._display.vvvvv('PWSH START', host=self._winrm_host)
        resptuple = self.execute_cmd("pwsh -ExecutionPolicy Bypass -NoProfile -NoLogo -File {} -HostName {} -UserName {} -Password {} -ScriptFile {}".format(ps_wrapper_filename, self._winrm_host, self._winrm_user, self._winrm_pass, ps_target_filename))
        self._display.vvvvv('PWSH END', host=self._winrm_host)
        
        #Remove the temp files used
        os.remove(ps_wrapper_filename)
        os.remove(ps_target_filename)
        result = Response(tuple(to_text(v) if isinstance(v, binary_type) else v for v in resptuple))
        ...
        self._display.vvvvv('RESULT STDOUT: "%s"' % (result.std_out), host=self._winrm_host)
